...
type OverlayType = 'pilih' | 'perpanjang' | 'tebus'
...
function PilihTipeSheet({ onSelect, onClose }: {
  onSelect: (t: 'perpanjang' | 'tebus') => void
  onClose: () => void
}) {
  const options = [
    {
...
// ── Overlay: Perpanjang ───────────────────────────────────────────────────────
function PerpanjangSheet({ onBack, onClose }: { onBack: () => void; onClose: () => void }) {
    ...
    <>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-2xl px-4 pb-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-[18px] font-semibold text-[#020617]">Perpanjang Pinjaman</p>
            <p className="text-[14px] text-[#64748b] leading-5">
              Mohon untuk memastikan data transaksi kembali sebelum lanjut ke halaman pembayaran.
            </p>
          </div>
          <button onClick={() => setCicil(v => !v)} className="flex items-center gap-3">
            ...
            <span className="text-[14px] font-medium text-[#020617]">Tambah Cicil Pinjaman</span>
          </button>
          <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-[8px] px-4 py-3 flex flex-col gap-3">
            <div className="flex justify-between">
              <span className="text-[14px] text-[#64748b]">Nilai Pinjaman</span>
              <span className="text-[14px] text-[#64748b]">Rp 850.000</span>
            </div>
            <div className="h-px bg-[#e2e8f0]" />
            <div className="flex flex-col gap-1">
              <div className="flex justify-between">
                <span className="text-[14px] text-[#64748b]">Biaya Jasa (10%)</span>
                <span className="text-[14px] text-[#64748b]">Rp 85.000</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[14px] text-[#64748b]">Biaya Keterlambatan (5%)</span>
                <span className="text-[14px] text-[#64748b]">Rp 0</span>
              </div>
            </div>
            <div className="h-px bg-[#e2e8f0]" />
            <div className="flex justify-between items-center">
              <span className="text-[14px] font-semibold text-[#020617]">Total Pembayaran</span>
              <span className="text-[18px] font-semibold text-[#023dff]">Rp 85.000</span>
            </div>
            <div className="h-px bg-[#e2e8f0]" />
            <div className="flex justify-between items-center">
              <span className="text-[14px] text-[#64748b]">Kamu dapat Poin Pandai</span>
              <div className="flex items-center gap-1">
                <img src={imgPoinFill} alt="" className="size-4" />
                <span className="text-[14px] font-bold text-[#b27202]">2.000</span>
              </div>
            </div>
          </div>
          <button className="w-full bg-[#023dff] rounded-[6px] py-[10px] flex items-center justify-center gap-2">
            <span className="text-[14px] font-medium text-white">Perpanjang Pinjaman</span>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M2 7h10M8 3l4 4-4 4" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
          <button onClick={onBack} className="w-full py-[10px] border border-[#e2e8f0] rounded-[6px] text-[14px] font-medium text-[#020617]">
            Batal
          </button>
        </div>
      </div>
...
// ── Overlay: Tebus ────────────────────────────────────────────────────────────
function TebusSheet({ onBack, onClose }: { onBack: () => void; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl shadow-2xl px-4 pb-6">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-1">
            <p className="text-[18px] font-semibold text-[#020617]">Tebus Pinjaman</p>
            <p className="text-[14px] text-[#64748b] leading-5">
              Pastikan kamu telah membaca syarat & ketentuan sebelum lanjut menebus barang.
            </p>
          </div>
          <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-[8px] p-3 flex items-start gap-3">
            <div className="size-14 rounded-xl bg-[#cfe7ff] flex items-center justify-center shrink-0">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#023dff" strokeWidth="1.5" strokeLinecap="round">
                <rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18" strokeWidth="2"/>
              </svg>
            </div>
            <div className="flex flex-col gap-0.5 flex-1">
              <p className="text-[14px] font-semibold text-[#020617]">
          ...
          <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-[8px] px-4 py-3 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex justify-between">
                <span className="text-[14px] text-[#64748b]">Biaya Tebus Pinjaman</span>
                <span className="text-[14px] text-[#64748b]">Rp 850.000</span>
              </div>
            </div>
            <div className="h-px bg-[#e2e8f0]" />
            <div className="flex justify-between items-center">
              <span className="text-[14px] font-semibold text-[#020617]">Total Pembayaran</span>
              <span className="text-[18px] font-semibold text-[#023dff]">Rp 850.000</span>
            </div>
            <div className="h-px bg-[#e2e8f0]" />
            <div className="flex justify-between items-center">
              <span className="text-[14px] text-[#64748b]">Kamu dapat Poin Pandai</span>
          ...
          <button onClick={() => setAgreed(v => !v)} className="flex items-center gap-2 text-left">
            ...
            <span className="text-[12px] text-[#020617]">
              Saya menyetujui <span className="font-bold text-[#023dff]">Syarat & Ketentuan</span> berlaku
            </span>
          </button>
          <button
            className={`w-full rounded-[6px] py-[10px] flex items-center justify-center gap-2 ${agreed ? 'bg-[#023dff]' : 'bg-[#023dff]/40'}`}
            disabled={!agreed}
          >
            <span className="text-[14px] font-medium text-white">Tebus Pinjaman</span>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M2 7h10M8 3l4 4-4 4" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
...
export default function SBGDetail() {
  const navigate = useNavigate()
  const [overlay, setOverlay] = useState<OverlayType>('pilih')

  return (
    <BasePage onBayar={() => setOverlay('pilih')}>
      {overlay === 'pilih' && (
        <PilihTipeSheet
          onSelect={(t) => navigate(t === 'perpanjang' ? '/pinjaman/perpanjang' : '/pinjaman/tebus')}
          onClose={() => navigate(-1)}
        />
      )}
    </BasePage>
...
export function SBGPerpanjang() {
  const navigate = useNavigate()
  return (
    <BasePage onBayar={() => navigate('/pinjaman/sbg-detail')}>
      <PerpanjangSheet
        onBack={() => navigate('/pinjaman/sbg-detail')}
        onClose={() => navigate(-1)}
      />
    </BasePage>
  )
