...
function Toggle({ on, onClick }: { on: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-11 h-6 rounded-full transition-colors shrink-0 flex items-center ${on ? 'bg-[#023dff]' : 'bg-[#e1e7ef]'}`}
    >
      <div className={`size-5 rounded-full bg-white shadow transition-transform mx-0.5 ${on ? 'translate-x-5' : 'translate-x-0'}`} />
    </button>
  )
}

function IconPoinEmas() {
  return (
    <div className="size-7 rounded-full bg-[#fffdc6] flex items-center justify-center shrink-0">
...
export default function PinjamanCicil() {
  ...
  const [cicilOn, setCicilOn] = useState(true)
    ...
    <div className="w-[375px] flex flex-col bg-white overflow-hidden rounded-3xl shadow-2xl" style={{ height: 812 }}>
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar px-4 py-4 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <div className="flex border border-[#cbd5e1] rounded-[6px] overflow-hidden">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-3 text-[14px] text-[#64748b] outline-none"
                />
              </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-lg px-4 py-3 flex flex-col gap-3">
            <div className="flex items-start justify-between">
              ...
              <span className="text-[14px] text-[#020617]">Rp85.000</span>
            </div>
            <div className="h-px bg-[#e2e8f0]" />
            <div className="flex items-center justify-between">
              <span className="text-[14px] font-semibold text-[#020617]">Subtotal</span>
              <span className="text-[16px] font-semibold text-[#023dff]">Rp85.000</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3">
          <div>
            ...
            <p className="text-[12px] text-[#64748b] leading-[18px]">Gunakan promo dan poin untuk kurangi total pembayaran.</p>
          </div>
          ...
          {/* Poin Pandai toggle */}
          <div className="border border-[#e1e7ef] rounded-lg px-3 py-2">
            <div className="flex items-center gap-3 py-2">
              <IconPoinEmas />
              <div className="flex-1">
                <div className="flex items-center gap-1">
                  <span className="text-[14px] font-medium text-black">Poin Pandai</span>
                ...
                <p className="text-[12px] text-[#64748b]">Poin tersedia: 12.000</p>
              </div>
              <Toggle on={poinOn} onClick={() => setPoinOn(v => !v)} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="shrink-0">
        ...
        <div className="flex items-center justify-center gap-1 h-8"
          ...
          <span className="text-[14px] font-semibold text-[#94590a]">2.000</span>
        </div>
        ...
        <div className="bg-white px-4 py-4 flex items-center gap-2">
          <div className="flex-1">
            ...
            <p className="text-[20px] font-bold text-[#023dff]">Rp85.000</p>
          </div>
          <button className="bg-[#023dff] rounded-[6px] px-4 py-2 flex items-center gap-2">
