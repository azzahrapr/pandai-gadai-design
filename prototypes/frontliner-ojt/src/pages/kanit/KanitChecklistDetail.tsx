...
export default function KanitChecklistDetail() {
    ...
    return (
      <div className="p-8 flex items-center justify-center min-h-96">
        <div className="text-center">
          ...
          <Link to="/kanit/review" className="mt-4 inline-flex items-center gap-2 text-sm text-[#023DFF] hover:underline">← Kembali ke Review</Link>
        </div>
      </div>
    )
  }
  ...
  const scoreColor = clScore >= 85 ? 'text-[#16A34A]' : clScore >= 75 ? 'text-[#B27202]' : 'text-[#DC2626]'
  const scoreBg = clScore >= 85
    ? 'bg-[#F0FDF4] border-[#16A34A]/20'
    : clScore >= 75
    ? 'bg-[#FEFDEA] border-[#E0A200]/30'
    : 'bg-[#FEF2F2] border-[#DC2626]/20'
  const scoreLabelColor = clScore >= 85 ? 'text-[#15803D]' : clScore >= 75 ? 'text-[#B27202]' : 'text-[#B91C1C]'
  const scoreNoteColor = clScore >= 85 ? 'text-[#15803D]' : clScore >= 75 ? 'text-[#92400E]' : 'text-[#B91C1C]'

  return (
    <div className="p-8 max-w-2xl">
      {/* Breadcrumb */}
      <div className="flex items-center gap-2 text-sm text-[#65758B] mb-6">
        <Link to="/kanit/review" className="hover:text-[#023DFF] transition-colors">Review Checklist</Link>
        <span>/</span>
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-base font-bold flex-shrink-0 ${
          clScore >= 85 ? 'bg-[#F0FDF4] text-[#15803D]' : clScore >= 75 ? 'bg-[#FEFDEA] text-[#B27202]' : 'bg-[#FEF2F2] text-[#B91C1C]'
        }`}>{cl.day}</div>
        <div>
          <h1 className="text-2xl font-bold text-[#0F1729]">Hari {cl.day}</h1>
          <p className="text-sm text-[#65758B] mt-0.5">{flUser?.name} · {cl.date}</p>
        </div>
        <span className="ml-auto text-xs font-semibold px-2.5 py-1 rounded-full bg-[#F0FDF4] text-[#15803D]">Sudah Dinilai</span>
      </div>

      {/* Score card */}
      {cl.kanitScore !== undefined && (
        <div className={`rounded-xl p-4 mb-6 flex items-center gap-4 border ${scoreBg}`}>
          <div className="flex-1">
            <p className={`text-xs font-semibold uppercase tracking-wide mb-1 ${scoreLabelColor}`}>Nilai dari Kanit</p>
            {cl.kanitNote
              ? <p className={`text-sm leading-relaxed ${scoreNoteColor}`}>"{cl.kanitNote}"</p>
              : <p className="text-sm text-[#94A3B8] italic">Tidak ada catatan</p>
            }
          </div>
          <p className={`text-4xl font-black flex-shrink-0 ${scoreColor}`}>{cl.kanitScore}</p>
        </div>
      )}
