...
export default function FLTaskDetail() {
  ...
  const taskHistory = allChecklists
    .filter(cl => cl.tasks?.some(t => t.taskId === taskId))
    ...
    return (
      <div className="p-8 flex items-center justify-center min-h-96">
        <div className="text-center">
          ...
          <p className="text-[#65758B]">Task tidak ditemukan</p>
  ...
  const scores = taskHistory.map(cl => {
    const t = cl.tasks!.find(t => t.taskId === taskId)!
    return t.kanitScore ?? null
  })
  const scoredList = scores.filter((s): s is number => s !== null)
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center gap-2 text-sm text-[#65758B] mb-6">
        ...
        <span>/</span>
        <span className="text-[#0F1729]">{task.name}</span>
      </div>

      {/* Header */}
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#0F1729]">{task.name}</h1>
          <p className="text-sm text-[#65758B] mt-1">{task.items.length} item · Hari ke-{profile.currentDay} dari 14</p>
        </div>
        {avg !== null && (
        ...
        <div className="bg-white rounded-xl border border-[#E1E7EF] p-4 mb-6">
          ...
          <div className="flex items-end gap-2">
            {taskHistory.map((cl, idx) => {
              const t = cl.tasks!.find(t => t.taskId === taskId)!
              const s = t.kanitScore ?? null
              const c = scoreColor(s)
              const maxScore = 100
              const barH = s !== null ? Math.max(16, Math.round((s / maxScore) * 64)) : 8
              return (
                <div key={cl.id} className="flex flex-col items-center gap-1 flex-1">
                  ...
                  <div className="w-full flex items-end justify-center" style={{ height: 64 }}>
                    <div
                      className={`w-full rounded-t ${s !== null ? c.bg.replace('border', '').trim() : 'bg-[#F1F5F9]'}`}
                      style={{ height: barH }}
                    />
                  </div>
      ...
      {taskHistory.length === 0 ? (
        ...
        <div className="space-y-4">
          {[...taskHistory].reverse().map(cl => {
            const t = cl.tasks!.find(t => t.taskId === taskId)!
            const s = t.kanitScore ?? null
            const c = scoreColor(s)
            const isScored = cl.status === 'scored' && s !== null
            return (
                ...
                <div className="px-5 py-3.5 bg-[#F8FAFC] border-b border-[#E1E7EF] flex items-center gap-3">
                  ...
                  {isScored ? (
                    <div className="flex items-center gap-2">
                      <span className={`text-xl font-black ${c.text}`}>{s}</span>
                ...
                <div className="p-5 space-y-4">
                  ...
                  {isScored && t.kanitNote && (
                    <div className={`rounded-lg px-4 py-3 ${c.bg}`}>
                      ...
                      <p className={`text-sm leading-relaxed italic ${c.text}`}>"{t.kanitNote}"</p>
                    </div>
                  )}

                  {/* Checklist items */}
                  <div>
                    <p className="text-xs font-semibold text-[#65758B] uppercase tracking-wide mb-2">Item Diselesaikan ({t.completedItemIds.length}/{task.items.length})</p>
                    <div className="space-y-2">
                      {task.items.map(item => {
                        const done = t.completedItemIds.includes(item.id)
                        return (
                  ...
                  {t.reflection && (
                    <div className="pt-3 border-t border-[#E1E7EF]">
                      ...
                      <p className="text-sm text-[#0F1729] italic leading-relaxed">"{t.reflection}"</p>
                    </div>
                  )}
                </div>
              </div>
